...
# create_time : 2018-06-20

# update_time: 2018-06-29
# update:
#    calcwoe sub_iv inf
#    
...
# updata 2018-06
def vars_rough_woe_iv(data, feature, path,sub_path,ifsave=False,bins=11):
    """
...
def save_var_bins_woe_iv(df,cal,path,sub_path):
    ...
    df.to_excel(path + '/models/' +sub_path +'/%s_woe.xls' % col0,header=True)
...
def ChiMerge_MinChisq(df, col, target, confidenceVal=5, init=11):
    ...
    if df[col].isnull().sum() > 0:
        ...
        woe = np.log(good_pcnt * 1.0 / bad_pcnt) if good_pcnt != 0 and bad_pcnt !=0 else 0
        iv_null = (good_pcnt-bad_pcnt)*np.log(good_pcnt*1.0/bad_pcnt) if good_pcnt != 0 and bad_pcnt !=0 else 0
        woe_df_null = [np.nan, total, bad, good,good_pcnt, bad_pcnt, woe,odds,total_pct,iv_null]
        # print woe_df_null
    ...
    if len(colLevels) > init:
        cutOffPoints = list(set(df_notnull[col].quantile(np.linspace(0.05,0.95,num=init))))
        cutOffPoints.append(-np.inf)
        cutOffPoints.append(np.inf)
        cutOffPoints.sort()
        print('cutOffPoints :')
        print(cutOffPoints)
        # df_notnull[col+'_cut'],tmp2 = pd.cut(df_notnull[col], bins=cutOffPoints, retbins=True,right=False)
        df_notnull[col+'_cut'],tmp2 = pd.cut(df_notnull[col], bins=cutOffPoints, retbins=True)
    ...
    while(1):
        ...
        if len(groupIntervals) == 1:
            ...
            cutOffPoints.sort()
            # df_notnull[col + '_cut'] = pd.cut(df_notnull[col], cutOffPoints, right=False)
...
def ChiMerge_MinChisq_MaxInterval(df, col, target, max_interval,confidenceVal=5,init=11):
    print('the var for bins is %s' % col)
    print('confidenceVal is %d' % confidenceVal)
    print('init is %d' % init)
    if df[col].isnull().sum() > 0:
    ...
    if monotone or (groupNum <= max_interval):
        ...
        return ChiMerge_MinChisq_MaxInterval(df, col, target, max_interval,confidenceVal+10, init)
...
def vars_careful_woe_iv_2(data, feature,max_interval, confidenceVal=1,sub_path='careful_woe',init=30,path='./',ifsave=False):
    cal = pd.DataFrame(index=['iv', 'cut_count', 'monotone'])
    woe_dict = {}
    for i in feature:
        if data[i].dtype == object:
            ...
            data[i + '_cut'], cutpoint, woe_df, iv_mono,data[i + '_woe'],WOE_dict_new = ChiMerge_MinChisq_MaxInterval(data, i, 'tag', max_interval,confidenceVal, init)
            # print(WOE_dict_new)
            if data[i + '_woe'].isnull().sum() > 0:
                data.ix[data[i + '_woe'].isnull(),i + '_woe'] = WOE_dict_new[np.nan]
        # k = ks(data, i + '_cut')
...
def model_evaluate_psi(df_train,col,df_test,cut_type='cut'):
	""" 违约概率十等分 - 等宽 或者 等深
	"""
	expect_data = df_train[col].fillna(-99999999)
	...
	actual_data = df_test[col].fillna(-99999999)
	if cut_type == 'cut':
...
def var_toushi_based_iv(col,woe_dict):
    df = woe_dict[col]
    fig=plt.figure(figsize=(12,8))
    ax1 = fig.add_subplot(511)
    internals = [col for col in woe_dict_0_v1_1[col][col + '_cut'].values]
    cnt = len(internals)
    # print(internals)
    ax1.plot(range(cnt), df['sub_iv'].values, 'r',label='sub iv distr')
    # plt.xticks(range(cnt),internals,fontsize=8,rotation=75)
    ax1.set_title("the var :  {0}".format(col))
    ax1.set_xticks([])
    ax1.legend()
    
    ax2 = fig.add_subplot(512)
    ax2.plot(range(cnt),df['WOE'].values,'b',label='woe distr')
    ax2.set_xticks([])
    ax2.legend()
    
    
    ax3 = fig.add_subplot(513)
    ax3.plot(range(cnt),df['total_bad_pct'].values,'g',label='total_bad_pct distr')
    ax3.set_xticks([])
    ax3.legend()
    
    
    ax4 = fig.add_subplot(514)
    ax4.plot(range(cnt),df['odds'].values,'b',label='odds distr')
    ax4.set_xticks([])
    ax4.legend()
    
    ax5 = fig.add_subplot(515)
    ax5.plot(range(cnt),df['total_pct'],label='total_pct distr')
    plt.xticks(range(cnt),internals,fontsize=8,rotation=75)
    ax5.legend()
    plt.show()
...
def all_vars_toushi_based_iv(cal,iv_value,woe_dict):
    cal_cols = list(cal[cal['iv'] > iv_value]['name'].values)
    print('cols for plot nums is {0}'.format(len(cal_cols)))
    for col in cal_cols:
        var_toushi_based_iv(col,woe_dict)
